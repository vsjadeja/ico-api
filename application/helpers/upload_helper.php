...
if (!function_exists('getDP')) {

    function getDP($dp = null) {
	$CI = & get_instance();
	$CI->load->config('cdn');
	...
	if ($dp):
	    if ($CI->config->item('is_real')):
		$CI->load->library('Awsts');
		$CI->load->config('aws');
		$bucket = $CI->config->item('pub-bucket');
		$aws = new Awsts();
		$s3 = $aws->getS3();
		if (file_exists("s3://" . $bucket . "/" . $uploadDir . $dp)):
		    return $s3->getObjectUrl($bucket, $uploadDir . $dp);
		else:
		    return $s3->getObjectUrl($bucket, $uploadDir . 'default.jpg');
		endif;
	    else:
		$file = $cdn . $uploadDir . $dp;
...
if (!function_exists('getLogoUrl')) {
    ...
    function getLogoUrl($thumb = null) {
	$CI = & get_instance();
	$CI->load->config('cdn');
	$cdn = $CI->config->item('cdn');
	$uploadDir = $CI->config->item('thumb_path');
...
if (!function_exists('getTextImageUrl')) {

    function getTextImageUrl($thumb = null) {
	$CI = & get_instance();
	$CI->load->config('cdn');
	$cdn = $CI->config->item('cdn');
	$uploadDir = $CI->config->item('thumb_path');
