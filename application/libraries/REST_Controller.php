...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest') {
        ...
        /**
         * Execute TS Security Layer
         */
        if ($this->config->item('rest_enable_content_verification')):
            $headers = $this->input->request_headers();
            $hash = (@$headers[$this->config->item('rest_bearer_name')]) ? $headers[$this->config->item('rest_bearer_name')] : 0;
            ...
            $newString = json_encode($params);
            if ($newString):
                $newHash = hash_hmac('sha256', $newString, $this->rest->pkey);
                if ($newHash !== $hash):
                    $this->response(NULL, self::HTTP_UNAUTHORIZED);
                endif;
            endif;
        endif;
    }
    ...
    protected function _detect_api_key() {
        ...
        $api_time_key_variable = $this->config->item('rest_timestamp_name');
...
//        //timestamp not match
//        $headers = $this->input->request_headers();
//        if ((time() - (int) @$headers[$api_time_key_variable]) > 60):
//            return FALSE;
//        endif;
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name))) {
            ...
            $this->rest->pkey = $row->{$this->config->item('rest_pkey_column')};
